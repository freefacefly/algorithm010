package io.kvh.algorithm.w4.c9.word_laddar;
...
//        List<String> wordList = new ArrayList<>(Arrays.asList("hot", "dot", "dog", "lot", "log", "cog"));
        ...
        int len = new SolutionT0().ladderLength(beginWord, endWord, wordList);
    ...
    public int ladderLengthV1(String beginWord, String endWord, List<String> wordList) {
        ...
        wordList.forEach(word -> {
            for (int i = 0; i < wordLen; i++) {
                ...
                List<String> combo = allComboDict.getOrDefault(newWord, new LinkedList<>());
                combo.add(word);
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < wordLen; i++) {
                String newWord = word.substring(0, i) + "*" + word.substring(i + 1, wordLen);
                for (String adjacent : allComboDict.getOrDefault(newWord, new LinkedList<>())) {
    ...
    public int ladderLengthV0(String beginWord, String endWord, List<String> wordList) {
        ...
        this.dict = new HashSet<>(wordList.size());
        ...
        while (!queue.isEmpty()) {
            int size = queue.size();
            for (int j = 0; j < size; j++) {
                String toFind = queue.poll();

                visited.add(toFind);

                if (toFind.equals(endWord)) {
                    return level;
                }
    ...
    /**
     * deprecated
     * dfs here is not suitable
     *
     * @param toFind
     * @param level
     * @return
     */
    private int dfs(String toFind,
                    int level) {
